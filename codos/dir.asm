...
START:      lda     #$00            ; Unprotects SYSRAM
            sta     HSRCW           ;
            cld                     ;
...
GPATTERN:   jsr     GETPATTERN      ; Get search pattern and disk from command line

DIRSEARCH:  jsr     SETOUTBCH       ; Set output buffer to output line buffer and sets
...
;
PRINTFILE:  lda     #' '            ; Clear output buffer with blanks
...
;
COPYFSIZEH: lda     #'$'            ; Copy the hex indicator to output buffer
            sta     (OUTBUFP),y     ;
            iny                     ; Advance one pos
...
; Y contains current output buffer position
;
COPYFSIZEL: lda     DIRBUF,x        ; Get byte
            jsr     HEXBYTE         ; And copy to the output buffer as ascii hex
            rts

; Get file pattern and drive from command line
;
GETPATTERN: ldx     DEFDRV          ; Set default drive as current drive
            stx     CURRDRV         ;
...
;
ISMATCH:    ldx     #$00            ; Init index to file name char position
...
;
DEFPATTERN: lda     #'*'            ; Store pattern into file name
            sta     FNAME           ;
            lda     #'.'            ;
            sta     FNAME+1         ;
            lda     #'?'            ;
