...
;
.proc CLRINITLN
            ldy     #DUMPLINELEN    ; Clear output buffer
            lda     #' '            ; with spaces
LOOP:       sta     (OUTBUFP),y     ;
            dey                     ;
            bpl     LOOP            ;
...
;
.proc INITLN
            lda     DUMPBYTES       ; Init number of bytes to dump per display line
            sta     BYTESLEFT       ;
...
;
.proc NXTBYTE
            inc     TMPBUFP         ; Go to address of next byte to dump
            bne     CONT            ;
            inc     TMPBUFP+1       ;
CONT: 
            ; Fall through
...
;
.proc ONELESS
            dec     BYTESPACE       ; Check if an extra space is needed
            bne     NOEXTRA         ; Not yet
