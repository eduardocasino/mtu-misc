...
;
GETARGS:    lda     #DEFINTR<<2     ; Set default interleave
...
; One drive operation
;
ONEDRIVE:   bit     ODRIVES         ; Check if drive 0 is open
...
TWODRIVES:  bit     ODRIVES+1       ; Check if drive 1 is open
            bpl     @CLOSED         ; No, continue
...
;
COPYOVLS0:  jsr     PUTSRCD         ; Ask for source disk in drive 0
...
;
COPYFILE1:  ldx     #$00            ; Set current drive 0
            stx     CURRDRV         ;
...
;
COPYFILE0:  lda     #$00            ; Init file size and file position
            ldx     #$05            ;
@INIT:      sta     SFILPOS,x       ;
            dex                     ;
            bpl     @INIT           ;
...
;
CPYFNAM:    ldy     #$00            ; Init index to name in FNAMBUF
...
;
PUTSRCD:    bit     ODRIVES         ; Check if drive 0 is open
...
;
PUTDSTD:    bit     ODRIVES         ; Check if drive 0 is open
            bpl     @CONT           ; No, continue
            ldx     #$00            ; Yes, close drive
            jsr     CLOSEDRV        ;
...
;
INITBAT:    stx     CURRDRV         ; Set current drive
            jsr     SETBATP         ; Set the BATP to the current drive's BAT
...
;
SETVOLID:   stx     CURRDRV         ; Set current drive
            jsr     SETBATP         ; Set the BATP to the current drive's BAT
...
;
INITDIR:    stx     CURRDRV         ; Set current drive
...
; Verify track X
;
VERIFY:     stx     VERTRACK        ; Set track to verify
            cld                     ;
            ldx     CURRDRV         ; Get current drive
...
;
MYERRRCVRY: sta     SAVEACC         ; Save accumulator
...
;          Cy set if not
;
BLOCKNUM:   lda     #$00            ; Init sector count
            sta     SECTCOUNT       ;
            sta     SECTCOUNT+1     ;
...
;
CHKRAND:    lda     #$00            ; Start with head 0
            sta     CHEAD           ;
